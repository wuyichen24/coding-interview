# Amazon

## 2021
### Online assessment (OA)
- [Optimizing Box Weights](../problems/array/Optimizing_Box_Weights.md)
- [Gifting Group (Get Number of Groups in Undirected Graph)](../problems/graph/Get_Number_Of_Groups_In_Undirected_Graph.md)

## 2023
### Online assessment (OA)
- [OA interview question bank](problems/other/amazon/README.md)

### Virtual onsite
- [Best Time to Buy and Sell Stock In 1 Transaction](../problems/array/stock/Best_Time_To_Buy_And_Sell_Stock_In_1_Transaction.md)
- [Climbing Stairs](../problems/other/Climbing_Stairs.md)
- [Data Structure with All Operations in O(1)](../problems/other/Data_Structure_With_All_O1_Operations.md)
